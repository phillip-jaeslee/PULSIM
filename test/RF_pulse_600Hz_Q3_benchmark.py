...
M_record[i], df_temp[i], RF_temp[i], RF_angle_temp[i], t_max_temp[i], Ns[i] =torch_pulse.torch_import_shaped_pulse(M, np.pi , "x", 0.27, file_path, BW, Gamma)
#M, df_temp[i], RF_temp[i], RF_angle_temp[i], t_max_temp[i], Ns[i] = torch_pulse.torch_hard_pulse(M, np.pi/2, "x", 0.02, N, BW, Gamma)

i = 1
file_path = 'wave/Q3_surbop.1'
M_record[i], df_temp[i], RF_temp[i], RF_angle_temp[i], t_max_temp[i], Ns[i] =torch_pulse.torch_import_shaped_pulse(M, np.pi , "x", 0.355, file_path, BW, Gamma)
...
i = 3
file_path = 'wave/Q3.1000'
M_record[i], df_temp[i], RF_temp[i], RF_angle_temp[i], t_max_temp[i], Ns[i] =torch_pulse.torch_import_shaped_pulse(M, np.pi , "x", 0.285, file_path, BW, Gamma)
...
df = df * 1000
axs.plot(df, M_record[0, 2, :], label="Q3_surbop-270us", color='red', alpha=1)
axs.plot(df, M_record[1, 2, :], label="Q3_surbop-355us", color='magenta')
axs.plot(df, M_record[2, 2, :], label="Q3_1000-270us", color='blue')
axs.plot(df, M_record[3, 2, :], label="Q3_1000-285us", color='green')
...
fig.savefig('arial_benchmark_600MHz_Q3.png', dpi=600)
