...
M_record[i], df_temp[i], RF_temp[i], RF_angle_temp[i], t_max_temp[i], Ns[i] =torch_pulse.torch_import_shaped_pulse(M, np.pi/2 , "x", 0.25, file_path, BW, Gamma)
#M, df_temp[i], RF_temp[i], RF_angle_temp[i], t_max_temp[i], Ns[i] = torch_pulse.torch_hard_pulse(M, np.pi/2, "x", 0.02, N, BW, Gamma)

i = 1
file_path = 'wave/Q5_sebop.1'
M_record[i], df_temp[i], RF_temp[i], RF_angle_temp[i], t_max_temp[i], Ns[i] =torch_pulse.torch_import_shaped_pulse(M, np.pi/2 , "x", 0.317, file_path, BW, Gamma)
...
i = 3
file_path = 'wave/Q5.1000'
M_record[i], df_temp[i], RF_temp[i], RF_angle_temp[i], t_max_temp[i], Ns[i] =torch_pulse.torch_import_shaped_pulse(M, np.pi/2, "x", 0.280, file_path, BW, Gamma)
...
df = df * 1000
axs.plot(df, M_record[0, 0, :], label="Q5_sebop-250us", color='red', alpha=1)
axs.plot(df, M_record[1, 0, :], label="Q5_sebop-317us", color='blue')
axs.plot(df, M_record[2, 0, :], label="Q5_1000-250us", color='green')
axs.plot(df, M_record[3, 0, :], label="Q5_1000-280us", color='magenta')
...
fig.savefig('arial_benchmark_850MHz_Q5_x.svg', dpi=600)
