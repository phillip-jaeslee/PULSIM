...
shape = RFShape.create("gausscasq5", duration=tp_ms, points=1000)
pulse_shape = Pulse(shape, PI / 2, axis = "x", backend=NumpyBackend(Gamma=GAMMA_H))
seg_shape = ShapePulseSegment(pulse_shape)

file_path = "wave/GaussCascadeQ5"
file_shape = RFShape.create("file", path=file_path, duration=tp_ms)
pulse_file = Pulse(file_shape, PI / 2, axis = "x", backend=NumpyBackend(Gamma=GAMMA_H))
seg_file = ShapePulseSegment(pulse_file)

def run_shape(sigma0, off_hz, J):
    off_H = twoPI * (off_hz / 1000.0)
    ss = SpinSystem(nuclei=['H', '15N'], offsets=[off_H, 0.0], couplings={(0, 1): J})
    seq = LiouvilleSequence([seg_shape], ss)
    return seq.propagate(sigma0)

def run_file(sigma0, off_hz, J):
    off_H = twoPI * (off_hz / 1000.0)
    ss = SpinSystem(nuclei=['H', '15N'], offsets=[off_H, 0.0], couplings={(0, 1): J})
    seq = LiouvilleSequence([seg_file], ss)
    return seq.propagate(sigma0)

def extract(sigma):
    name_ops = [('Ix',      embed(Ix(), 0, 2)),
                ('Iy',      embed(Iy(), 0, 2)),
                ('Iz',      embed(Iz(), 0, 2)),
...
shape_J = {k: np.zeros(N) for k in keys}
file_J = {k: np.zeros(N) for k in keys}


for n, off in enumerate(W_hz):
    r_shape = extract(run_shape(Iz0, off, J_HZ))
    r_file = extract(run_file(Iz0, off, J_HZ))
    for k in keys:
        shape_J[k][n] = r_shape[k]
        file_J[k][n] = r_file[k]
...
for k, color in zip(keys, ['red', 'blue', 'green', 'orange', 'cyan', 'olive']):
    ax[0].plot(W_hz, shape_J[k], '-', color=color, label=k)
...
for k, color in zip(keys, ['red', 'blue', 'green', 'orange', 'cyan', 'olive']):
    ax[1].plot(W_hz, file_J[k], '-', color=color, label=k)
...
print(f"On-resonance: with J, IxSz={shape_J['IxSz'][N//2]:.4f}; "
      f"with J=0, IxSz={file_J['IxSz'][N//2]:.4f} (should be ~0)")